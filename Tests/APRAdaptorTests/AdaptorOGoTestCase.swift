...
class AdaptorOGoTestCase: XCTestCase {
  ...
  func testEvaluateQueryExpression() {
  ...
  func testTableReflection() {
    do {
      ...
      if printResults {
        print("  id:   \(idAttr as Optional) " +
              "\(idAttr?.externalType as Optional)")
      }
  ...
  #if false // produces error, hm
  func testRecordAttachedSchema() {
    ...
    do {
      // fails:
      let person1 = try ds.findBy(id: 7777)
      if printResults { print("got person: \(person1)") }
      
      let person = Person.findBy(id: 7777)
  ...
  func testCodeSchemaWithJoinQualifier() {
    let db = Database(adaptor: adaptor)
    ...
    do { // relship qualifier fetch (fetch addresses of a specific person)
      let ds = ActiveDataSource<Address>(database: db)
  ...
  func testCodeSchemaWithRelshipPrefetch() {
    let db = Database(adaptor: adaptor)
  ...
  func testCodeSchemaWithTypedFetchSpec() {
    let db = Database(adaptor: adaptor)
